...
Avant de parler de l'IPIP NEO, il est important d'expliquer ce qu'est l'International Personality Item Pool (IPIP). D'après le site officiel de l'IPIP (https://ipip.ori.org/), celui-ci regroupe plus de 3000 items de plus de 250 échelles d'évaluation de la personnalité, construites à partir de ces items. Les items et échelles appartiennent au domaine public, ce qui signifie que n'importe qui peut les copier, éditer, traduire ou utiliser sans besoin d'autorisation, et sans avoir besoin de payer. En effet, Goldberg (1999) souligne que la plupart des instruments
évaluant la personnalité appartiennent à un auteur dont les items sont placés sous copyright, ce qui empêche les scientifiques de les utiliser gratuitement ou de les modifier. Cela réduit par exemple les possibilités des comparer deux instruments entre eux pour mesurer leur validité et leur performance. De plus, il souligne que les recherches
scientifiques sont alors soumises aux intérêts commerciaux. Il a donc entrepris de développer et de continuellement affiner une large bande d'inventaires de personnalité. Ainsi, L'International Personality Item Pool est né. L'IPIP présente de nombreux avantages. En effet, l'accès et l'utilisation est gratuite et rapide grâce à son site internet, il permet une certaine flexibilité car les utilisateurs peuvent administrer les items dans l'ordre qui leur convient et appliquer les modifications qu'ils désirent. De plus, il regroupe également un très grand nombre d'items qui permettent de développer des versions plus ou moins longues de différents tests (Goldberg et al., 2006). L'IPIP rassemble ainsi plus de 250 inventaires évaluant la personnalité dont le NEO PI R (Costa & McCrae, 1998), ici appelé : l'IPIP NEO.

# L'IPIP NEO

L'IPIP NEO est donc une « adaptation » du NEO PI, dans le sens où il vise à mesurer les mêmes domaines et les mêmes facettes. Ainsi à partir de 300 items, soit 10 items par facette, l'IPIP NEO évalue les cinq domaines du Big Five (Ouverture (O), caractère Consciencieux (C), Extraversion (E), Agréabilité-Altruisme (A), Névrosisme (N)). Comme pour
le NEO PI R, il se base également sur 6 facettes par domaine, qui, pour certaines, n'ont pas exactement le même intitulé, quand bien même les définitions ne sont pas différentes. Une proposition de traduction pour
les domaines et facettes sera présentée dans la suite de ce travail (Tableau 3).

Il fait l'objet de traductions dans plus de 10 langues, notamment en slovène (Musek, s.d.) en japonais (Karlin, s.d.) ou encore en français québécois (Gravel, s.d.). Le site de l'IPIP NEO permet à n'importe qui de passer le test gratuitement et d'obtenir, à la fin, un résultat détaillé et des informations sur sa personnalité. Ainsi, plus de 500 000 personnes ont déjà passé ce test depuis qu'il a été diffusé sur internet. Comme pour le NEO-PI, il existe des versions courtes de l'IPIP NEO avec seulement 120 items par exemple. Cela permet de répondre plus rapidement (10-20 minutes versus 30-40 minutes) mais donne des résultats moins précis que la version 300 items. Ainsi, sur la page internet « The IPIP NEO (International Personality Item Pool Representation of the NEO-PI-R) » (www.personal.psu.edu/~j5j/IPIP/ipipneo300.htm), n'importe qui peut remplir le questionnaire pour avoir un feedback concernant sa personnalité, dans sa version anglaise.
...
Pour cela, toute une procédure d'adaptation a été mise en place lors de la première étude menée pour ce travail. L'objectif serait en effet de se baser sur la traduction déjà existante de Gravel (s.d) disponible sur le site de l'IPIP NEO. Cette traduction a suivi les 3 première étapes de la méthode de Vallerand (1989). Cette procédure incluait une première traduction en français, suivi d'une back-translation en anglais, qui devait correspondre avec la version originale. De ce fait, certains items ont été modifié jusqu'à 3 fois pour correspondre au mieux et le pronom « Je » a dû être ajouté dans la version française. Ainsi, ce questionnaire a fait l'objet d'une traduction, mais jamais d'une
validation à proprement parler. C'est pourquoi il a été décidé, pour se baser sur un modèle traduit avec une méthode scientifique, de se baser sur cette traduction du questionnaire et d'envisager une adaptation de celui-ci.

# Question de recherche et hypothèses

La question de recherche générale de ce mémoire cherche à proposer une adaptation et une validation française de l'IPIP NEO.

En effet, en considérant 1) la non-existence d'une version française validée de l'IPIP NEO ; 2) ses avantages quant à sa qualité d'instrument « open-source » ; et 3) la potentielle facilitation du travail des chercheurs/étudiants par l'usage de ce test, l'objectif de ce travail va premièrement, tenter de proposer une adaptation adéquate de la version
française québécoise (Gravel, s.d) déjà existante et deuxièmement tenter de valider cette nouvelle version du questionnaire en se questionnant sur la fidélité (consistance interne) et la structure factorielle de cette nouvelle version.

## L'adaptation

L'objectif est ici de proposer une adaptation de la traduction française québécoise (Gravel s .d). Certains items semblent inadaptés ou difficilement compréhensibles en français européen. 
...
Cette première étude se concentre donc sur l'adaptation française de l'IPIP NEO, adaptation réalisée à partir de la version québécoise fournie par Gravel (s.d) sur le site de l'IPIP NEO. La procédure de l'étude ainsi que les résultats de celle-ci sont présentés ci-après.

### Participants

Les participants sont des étudiants en deuxième année de Master de Traduction et d'Interprétation au sein de l'Université de Mons en Belgique. Âgés entre 20 et 34 ans, avec une moyenne d'âge de 23,8 ans (ET = 3,43). 20 personnes ont participé, parmi lesquelles 15 femmes et 5 hommes. A ces 20 personnes s'ajoutent le directeur de mémoire ainsi que le mémorant qui sont intervenus dans la première étape de la procédure.

### Matériel

Les items analysés sont ceux de l'IPIP NEO 300 items, fourni par le site de l'IPIP et notamment, la version française québécoise fournie sur ce même site (Gravel, s.d). Le questionnaire a été réalisé sur Google Forms, une plateforme qui permet de faire passer des questionnaires en ligne gratuitement, permettant également d'assurer l'anonymat des
participants.[^1]
...
D'abord, le mémorant ainsi que son directeur de mémoire, ont parcouru la traduction française québécoise fournie sur le site de l'IPIP NEO, chacun de leur côté, afin d'identifier les items qui pourraient poser des problèmes de compréhension, tant au niveau du vocabulaire qu'au niveau des tournures de phrases. Les items ayant été estimés ambigus, sont les items pour lesquels le mémorant **et/ou** le directeur de mémoire pointaient un problème. Il est possible de noter que 91 % des items étaient consensuellement estimés comme non problématiques. Par contre, les 26 items suivants (9% des items) ont été identifiés comme étant potentiellement problématiques (Tableau 4).
...
"investis" ont été codées "I". De cette manière, il a alors été possible de comptabiliser les différents thèmes afin de dégager la traduction donnée la plus fréquemment par les participants, celle qui se rapproche le plus -*a priori*- de la meilleure traduction. Ainsi, pour cet item, le Tableau 5 a pu être dressé.
...
Ainsi, il est possible de dire que 75% des participants ont traduit "Work hard" par " Je travaille dur". C'est donc cette traduction qui sera retenue pour la version française de l'IPIP NEO.

Les mêmes analyses ont été réalisées sur chacun des items. Concernant les items 42, 163, 184, 191 et 267, ils ont en majorité été traduits de la même façon que pour la version québécoise et resteront donc inchangés.
...
Il est donc à présent possible de fournir une traduction française Européenne à l'IPIP NEO, en réutilisant la traduction québécoise validée et en replaçant tous les items ci-dessus par les nouvelles traductions fournies par notre panel de 20 traducteurs en devenir. A partir de cette étude 1, la suite de ce travail peut être entreprise.